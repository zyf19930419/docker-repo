...
class itunesReceiptValidator {
    ...
    function validateReceipt() {
            ...
            if ($errno != 0) {
                sendResponse(200, $errmsg);
            }

            $decoded_response = $this->decodeResponse($response);

            /*if (!isset($decoded_response->status) || $decoded_response->status != 0) {
                sendResponse(400, 'Invalid receipt. Status code: ' . (!empty($decoded_response->status) ? $decoded_response->status : 'N/A'));
            }

            if (!is_object($decoded_response)) {
                sendResponse(400, 'Invalid response data');
            }*/
